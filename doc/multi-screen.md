...
main.ts
  └─ RigidBodyPrecessionScreen            (Screen<RigidBodyPrecessionModel, RigidBodyPrecessionScreenView>)
       ├─ RigidBodyPrecessionModel         owns all state
       └─ RigidBodyPrecessionScreenView    owns all visuals
```
...
Mirror the structure of `src/precession-screen/`:
...
existing `RigidBodyPrecessionScreen.ts`. Screen icons live in one shared module under
